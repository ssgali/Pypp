...
namespace FSE_Project
{
    public partial class Form1 : Form
    {
        ...
        private string cppKeywords = "alignas alignof and and_eq asm atomic_cancel atomic_commit atomic_noexcept auto bitand bitor bool break case catch char char8_t char16_t char32_t class compl concept const constexpr const_cast continue co_await co_return co_yield decltype default delete do double dynamic_cast else enum explicit export extern false final float for friend goto if inline int #include long mutable namespace new noexcept not not_eq nullptr operator or or_eq private protected public register reinterpret_cast requires return short signed sizeof static static_assert static_cast struct switch synchronized template this thread_local throw true try typedef typeid typename union unsigned using virtual void volatile wchar_t while noexcept";
        private string cKeywords = "auto break case char const continue default do double else enum extern float for goto if inline int #include long register restrict return short signed sizeof static struct switch typedef union unsigned void volatile while _Alignas _Alignof _Atomic _Generic _Imaginary _Noreturn _Static_assert _Thread_local";
        public Form1()
        {
            InitializeComponent();
            string fontPath = Path.Combine(Application.StartupPath, "JetBrainsMonoNL-Regular.ttf");
            string logoPath = Path.Combine(Application.StartupPath, "logo.ico");

            pfc.AddFontFile(fontPath);
            jetBrainsMonoFamily = pfc.Families[0];

            this.Text = "Py++";
            this.Icon = new Icon(logoPath);
